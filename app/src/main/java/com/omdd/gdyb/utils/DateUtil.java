...
public class DateUtil {
    ...
    public static String getFilePath() {
        ...
        if (Environment.MEDIA_MOUNTED.equals(Environment.getExternalStorageState())
            ...
            cachePath = BaseApplication.getInstance().getExternalCacheDir();
        }
        if(cachePath == null) {
            cachePath = BaseApplication.getInstance().getCacheDir();
        }

        return cachePath.getPath() + "/" + fileFormat.format(new Date()) + "/" + (new GregorianCalendar().get(GregorianCalendar.AM_PM) == 0 ? "am" : "pm");
    }
}
